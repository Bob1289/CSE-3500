...
def encode(msg):        
    ...
    #frequency of char and char tuples in heap
...
def decode(cmsg, decoderRing):
    ...
    #invert padding
    cmsg = cmsg[:-1]
    #convert bytes into string format
    cmsg = ''.join(format(x, '08b') for x in cmsg)
    #decode msg
    byteMsg = bytearray()
    curr = ''
...
def ibwt(msg):
    ...
    original = bytearray()
    for i in range(len(msg)):
        original.append(first_col[index][0])
