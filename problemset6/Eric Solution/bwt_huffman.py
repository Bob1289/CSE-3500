...
# Create a Huffman tree using list of tuples (char, freq)
def encode(msg):        
    freq = {}
    for c in msg:
        if c in freq:
...
def decode(cmsg, decoderRing):
    compressedMsg = ""
    for c in cmsg:
        compressedMsg += bin(c)[2:].zfill(8)

    # Remove the padding from the encoded message using the decoder ring
    pad = decoderRing['padding']
    compressedMsg = compressedMsg[:-pad]

    # Decode the message using the decoder ring
    msg = bytearray()
    code = ""
    for c in compressedMsg:
        code += c
        if code in decoderRing:
            msg.append(decoderRing[code])
            code = ""
...
def compress(msg, useBWT): 
    ...
    # use to_bytes() and encode to convert the binary string into a bytearray
    binary = compressedMsg.encode()
    compressed = int(binary, 2).to_bytes((len(binary) + 7) // 8, byteorder='big')
